...
> vulnerable to race conditions: another process may remove the file between the calls
> to `fs.exists()` and `fs.open()`. Just open the file and handle the error when it's not there.
